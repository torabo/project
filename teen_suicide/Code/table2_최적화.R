...
d2024=read.csv("../teen_suicide/csv/kyrbs2024.csv")

# 데이터프레임 합치기
df <- dplyr::bind_rows(d2020, d2021, d2022, d2023, d2024)
# make variable names -----------------------------------------------------
#자살생각
y_idea <- df$M_SUI_CON 
#자살계획
...
# x,y-------------------------------------------------------------------------
df_merge = data.frame(y_idea, y_plan, y_attempt, x_sex, 
                      x_school_year, x_academic_grade, x_economic_status, 
                      x_living_with_family, x_health_status, 
...
# 독립 변수 목록 정의
independent_vars <- c(
  "smartphone_use_time_weekdays", "x_sex", "x_school_year", 'year', "x_living_with_family", "x_health_status", "x_sleep_status", "x_perceived_stress", "x_GAD", "x_loneliness", "x_violence")
...
common_colnames <- c(
  "Suicidal ideation_Dep", "95%CI(LOW)1", "95%CI(HIGH)1",
  "Suicide plan_Dep", "95%CI(LOW)2", "95%CI(HIGH)2",
  "Suicide attempt_Dep", "95%CI(LOW)3", "95%CI(HIGH)3",
  "Suicidal ideation_Nodep", "95%CI(LOW)4", "95%CI(HIGH)4",
  "Suicide plan_Nodep", "95%CI(LOW)5", "95%CI(HIGH)5",
  "Suicide attempt_Nodep", "95%CI(LOW)6", "95%CI(HIGH)6"
)
...
# 각 Dep/Nodep 열과 CI를 지정된 순서로 묶기
combined_table <- data.frame(
  Suicidal_ideation_Dep = paste0(
    table[, "Suicidal ideation_Dep"], 
    " (", table[, "95%CI(LOW)1"], "-", table[, "95%CI(HIGH)1"], ")"
  ),
  Suicide_plan_Dep = paste0(
    table[, "Suicide plan_Dep"], 
    " (", table[, "95%CI(LOW)2"], "-", table[, "95%CI(HIGH)2"], ")"
  ),
  Suicide_attempt_Dep = paste0(
    table[, "Suicide attempt_Dep"], 
    " (", table[, "95%CI(LOW)3"], "-", table[, "95%CI(HIGH)3"], ")"
  ),
  Suicidal_ideation_Nodep = paste0(
    table[, "Suicidal ideation_Nodep"], 
    " (", table[, "95%CI(LOW)4"], "-", table[, "95%CI(HIGH)4"], ")"
  ),
  Suicide_plan_Nodep = paste0(
    table[, "Suicide plan_Nodep"], 
    " (", table[, "95%CI(LOW)5"], "-", table[, "95%CI(HIGH)5"], ")"
  ),
  Suicide_attempt_Nodep = paste0(
    table[, "Suicide attempt_Nodep"], 
    " (", table[, "95%CI(LOW)6"], "-", table[, "95%CI(HIGH)6"], ")"
  ),
  row.names = rownames(table) # 기존 행 이름 유지
)

# 결과 확인
combined_table

#write.csv(combined_table, 'final2.csv')
#write.csv(table, 'final2table.csv')
